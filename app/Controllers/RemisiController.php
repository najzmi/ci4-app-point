...
class RemisiController extends BaseController
{
        ...
        if (! $this->request->is('post')) {
            ...
            if (! $this->validateData($data_req, $rules)) {
                ...
                $simpan_data = [
                    ...
                    'jml_remisi'     => $this->request->getPost('jml_remisi'),
                    'keterangan'     => 'Sudah dilakukan remisi sebanyak '.$this->request->getPost('jml_remisi').' %',
                ];
    ...
    public function edit($id)
    {
        ...
        // JML REMISI (harus di revisi)
        $resRemisi = ['15'=>'Remisi 15%', '25'=>'Remisi 25%', '50'=>'Remisi 50%'];
        $this->data['jml_remisi'] = form_dropdown('jml_remisi', $resRemisi, $data->jml_remisi, ['id'=>'jml_remisi', 'class'=>'form-control']);
        ...
        if (! $this->request->is('post')) {
            ...
            if (! $this->validateData($data_req, $rules)) {
                ...
                $data_update = [
                    'id_murid'       => $this->request->getPost('id_murid'),
                    'tanggal'        => $this->request->getPost('tanggal'),
                    'jml_remisi'     => $this->request->getPost('jml_remisi'), // perlu disesuaikan
                    'keterangan'     => 'Sudah dilakukan remisi sebanyak '.$this->request->getPost('jml_remisi').' %',
                ];
